...
## 저장된 파일 활용 (분석용 압축 · 대본 추출)

하단 **"저장된 파일"** 섹션에서 녹화물마다:

- **📉 분석용(≤500MB)** — 영상 길이에 맞춰 비트레이트를 자동 계산해 **500MB 미만**으로 재인코딩.
  Claude 등 업로드 용량 제한에 맞춥니다. (`_분석용.mp4` 생성)
- **📝 대본 추출** — 음성을 텍스트로 전사해 **`_대본.txt` / `_대본.srt`** 생성.
  영상 대신 **대본 텍스트를 Claude에 주면** 용량 걱정 없이 전체 내용을 분석시킬 수 있습니다.

### 대본 추출(STT) 상세

- 엔진: [faster-whisper](https://github.com/SYSTRAN/faster-whisper). **첫 사용 시 자동 설치**되고,
  모델(기본 `large-v3`)도 처음 한 번 다운로드됩니다.
